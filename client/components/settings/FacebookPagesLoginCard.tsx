...
const formatConnectedMessage = (connectedCount: number | null) => {
  if (!connectedCount) {
    return "Facebook Page login finished successfully.";
  }

  return `Connected ${connectedCount} Facebook Page${
    connectedCount === 1 ? "" : "s"
  }.`;
};

const getFallbackWebhookUrl = (callbackUrl?: string) => {
  if (!callbackUrl) {
    return "";
  }
  ...
  try {
    ...
    return `${url.origin}/api/webhooks/facebook`;
  } catch {
    return "";
  }
};

export function FacebookPagesLoginCard({
  callbackUrl,
  webhookUrl,
  ...
  const resolvedWebhookUrl = webhookUrl || getFallbackWebhookUrl(callbackUrl);
  ...
  return (
    <section className="glass-panel soft-ring rounded-[30px] border border-[var(--color-line)] p-5 md:p-6">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-2 text-sm leading-7 text-[var(--color-muted)]">
            Sellers can authorize their Facebook account, choose Pages, and let
            this inbox receive Page messages automatically.
          </p>
        </div>
      </div>
      ...
      <div className="mt-4 rounded-[24px] border border-[var(--color-line)] bg-white/80 p-4">
        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[var(--color-muted)]">
        ...
        <input
          type="text"
          readOnly
          value={loading ? "Loading webhook URL..." : resolvedWebhookUrl}
          className="mt-3 w-full rounded-2xl border border-[var(--color-line)] bg-slate-50 px-4 py-3 text-sm text-[var(--color-foreground)] outline-none"
        />
        <p className="mt-3 text-xs leading-6 text-[var(--color-muted)]">
          Facebook sends incoming Page messages to this webhook. Add it in the
          Meta Webhooks product for the Page object and subscribe to the
          messages field.
        </p>
      </div>
      ...
      {verifyTokenConfigured === false ? (
        <div className="mt-4 rounded-[24px] border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-800">
          Set VERIFY_TOKEN on the server before verifying the Facebook webhook
          in Meta.
        </div>
      ) : null}
      ...
      {missingConfiguration.length > 0 ? (
        <div className="mt-4 rounded-[24px] border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-800">
          Set {missingConfiguration.join(", ")} on the server before starting
          Facebook login.
        </div>
      ) : null}
